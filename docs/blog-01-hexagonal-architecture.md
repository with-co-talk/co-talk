...
이 글은 Co-Talk 백엔드를 전통적인 레이어드에서 헥사고날 아키텍처(Ports & Adapters)로 리팩토링한 과정을 다룬다. 이슈 #19, #22, #24에 걸쳐 진행된 작업이었고, 패키지 구조 설계부터 ArchUnit으로 의존 방향을 강제하는 것까지, 현실적인 트레이드오프와 함께 정리해봤다.
